...
if ( $('#tblSelectedRequest td:contains(' + material + ')').length ) {
    ...
    $('#tblSummaryList').append('<tr><td><input type="hidden" name="txt_selectedMaterial[]" value="' + materialID + '"><input type="hidden" name="txt_selectedQuantity[]" value="' + quantity + '">' +  material + '</td><td>' + quantity + '</td><td><a href="#" class="btn_removeItem"><i class="fa fa-remove"> </i> <b>Remove</b></a><script type="text/javascript">$(".btn_removeItem").click(function () { $(this).closest("tr").remove(); });</script> </td></tr>');
...
//request - populate organzation list lookup
     ...
     success: function(data)
      {
        $('#txt_clients_org').empty();
        $('#txt_clients_org').append('<option value="">Select Type</option>');
        $.each(data, function(key, value) {
        $('#txt_clients_org').append("<option value='" + value.id + "'>" + value.organization_name + "</option>");
        });
      }   
    });  
...
if(clients_name == '' || clients_org == '' || txt_clients_designation =='' || clients_contact_no ==''){
...
// print preview function & layout
  ...
  docPrint.document.write('<br><div class="box-primary"><center><table class="table table-bordered"><tr style="border:2px solid black;"><td style="width:30%; border:2px solid black !important;"><table class="table table-borderless"><tr style="border:none !important; padding:0px !important;"><td style="width:35%; border:none !important; padding:0px !important;"> <br> <center> <b>Philippine <br> Commission <br> On Women</b> </center>  </td><td style="width:50%; border:none !important; padding:0px !important;"> <br> <center> <img src="/images/pcw_logo.jpg" alt="pcw_header" height="20%" width="80%"> </center> </td></tr></table></td><td style="width:70%; text-align:center; border:2px solid black;"> <br> <h2> <b>Request History Report</b> </h3> </td></tr></table></center></div>');   
  docPrint.document.write(divToPrint.innerHTML);
   docPrint.document.write('<br><br><br><br><br><br><br>');
  docPrint.document.write('<p align=center><font face=arial size=2> Philippine Commission on Women 1145 J.P. Laurel St., San Miguel Manila 1005 Philippines');
    docPrint.document.write('<br> &copy ' + ydate + " " + 'Philippine Commission on Women Updated' + " " + mdate + " " + ydate + '</font></p>');
  ...
  docPrint.document.write('<html><body onload="window.print()">');
  docPrint.document.write('<link rel="stylesheet" href="/AdminLTE/bootstrap/css/bootstrap.min.css">');
  docPrint.document.write('<br><div class="box-primary"><center><table class="table table-bordered"><tr style="border:2px solid black;"><td style="width:30%; border:2px solid black !important;"><table class="table table-borderless"><tr style="border:none !important; padding:0px !important;"><td style="width:35%; border:none !important; padding:0px !important;"> <br> <center> <b>Philippine <br> Commission <br> On Women</b> </center>  </td><td style="width:50%; border:none !important; padding:0px !important;"> <br> <center> <img src="/images/pcw_logo.jpg" alt="pcw_header" height="20%" width="80%"> </center> </td></tr></table></td><td style="width:70%; text-align:center; border:2px solid black;"> <br> <h2> <b>Printing Logs Report</b> </h3> </td></tr></table></center></div>');   
  docPrint.document.write(divToPrint.innerHTML);
   docPrint.document.write('<br><br><br><br><br><br><br>');
  docPrint.document.write('<p align=center><font face=arial size=2> Philippine Commission on Women 1145 J.P. Laurel St., San Miguel Manila 1005 Philippines');
    docPrint.document.write('<br> &copy ' + ydate + " " + 'Philippine Commission on Women Updated' + " " + mdate + " " + ydate + '</font></p>');
...
$(document).ready(function(){
  ...
  //most distributed IEC Materials
  var ayear = [];
  var atotal = [];
     $.ajax({
     url: "/admin/requests/graph/iec/org",
     dataType: "json", 
     success: function(data)
      { 
    for (var i in data) {
        ayear.push(data[i].iec_title);
        atotal.push(data[i].total_req);
    }
//   $.each(data, function(key, value) {
 
//       }); //each data 7-23-2020
var ctx = document.getElementById("iecMaterialsPerPROHorizontal").getContext('2d');
var myChart = new Chart(ctx, {
type: 'bar',
data: {
labels: ayear,
datasets: [{
label: 'Most Distributed IEC Materials Per Material',
data: atotal,
...
$(document).ready(function(){
//Number of IEC Materials on stock
  //most distributed IEC Materials
  var aw = [];
  var qe = [];
  var bcolor = [];
  var asd = [];
     $.ajax({
     url: "/admin/iecs/available-stock",
     dataType: "json", 
     success: function(data)
      {
        $('iecMaterialsPerPROHorizontal').css('display', 'block');
 //   $.each(data, function(key, value) {
    for (var i in data) {
        aw.push(data[i].iec_refno);
        qe.push(data[i].iec_threshold);
    }

 new Chart(document.getElementById("IECMaterialsStock"), {
"type": "horizontalBar",
"data": {
"labels": aw,
"datasets": [{
"label": "IEC Materials on stock",
"data": qe,
"fill": false,
...
//end number of iec materials on stock
  //most distributed IEC Materials
  var aw1 = [];
  var qe1 = [];
     $.ajax({
     url: "/admin/requests/graph/",
     dataType: "json", 
     success: function(data)
      {
 //   $.each(data, function(key, value) {
    for (var i in data) {
        aw1.push(data[i].iec_refno);
        qe1.push(data[i].total_req);
    }
1
 new Chart(document.getElementById("horizontalBarIECOnStock"), {
"type": "horizontalBar",
"data": {
"labels": aw1,
"datasets": [{
"label": "Most Distributed IEC Materials",
"data": qe1,
"fill": false,
          ...
          if(value.iec_update_pieces !=''){
            ...
            var iecUpdatePcs = '';
              ...
              var date = new Date(value.created_at);
               var created_date = date.getMonthName() + ' ' + ((date.getDate() > 9) ? date.getDate() : ('0' + date.getDate())) + ', ' + date.getFullYear();
                $('#iec_history_body_display').append('<tr><td>' + iecUpdateType + '</td><td>' + value.iec_update_threshold + '</td><td>' + pcs_sign + iecUpdatePcs + '</td><td>' + currentThreshold +'</td><td>' +  created_date  + '</td><td>' +  value.name + '</td><td>' + value.iec_update_remarks+ '</td></tr>');
       });
      } //endif 
    } 
    }); 
